...
fn main() -> Result<(), anyhow::Error> {
    ...
    let mut read_buff = vec![0; buf_address.unwrap().1];
    ...
    loop {
        ...
        if target != old_target {
            ...
            let br = bytes_to_read(host as usize, target as usize, 1024);

            let mut read = &mut read_buff[0..br];

            if host + br as u32 > 1024 {
                // cursor will overflow
                let pivot = host.wrapping_add(br as u32).wrapping_sub(1024) as usize;
                ...
                core.write_word_32(cursor_address.unwrap() + 4, (host + br as u32) % 1024)?;
            }
            let dur = now.elapsed();
...
fn bytes_to_read(host_idx: usize, target_idx: usize, buffer_size: usize) -> usize {
    // let cursor = host_idx;
    let len_to_read = (target_idx - host_idx + buffer_size) % buffer_size;
